...
void MRCC_voidInit()
{
	...
	//If HSE is on, then HSE bypass is Enable
	if(RCC_User_cfg.HSE_ENABLE == ENABLE)
		SET_BIT(RCC->CR,HSEBYP);

	CLR_BIT(RCC->CR,PLLON);
	RCC->CR |= (RCC_User_cfg.PLL_ENABLE)<<PLLON;

	CLR_BIT(RCC->CR,HSION);
	RCC->CR |= (RCC_User_cfg.HSI_ENABLE)<<HSION;
